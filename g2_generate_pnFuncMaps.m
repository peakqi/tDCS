function g2_generate_pnFuncMaps(CRS,lrap1,tp)
...
% tp='TP2'
    ...
    ntotal=9;
for ii=n1:nn
   namestrre=strcat('re_PosiNegFunc_',CRS,num2str(ii),'_',lrap1,'_',tp,'.nii.gz');
   namestrac=strcat('ac_PosiNegFunc_',CRS,num2str(ii),'_',lrap1,'_',tp,'.nii.gz');
    if ii==n1
...
qf_save(mask.*single((1-pmap)>0.95).*single(totmap/ntotal>0).*totmap/ntotal,strcat('pPosi_PosiNegFunc_',CRS,'_',lrap1,'_',tp,'.nii.gz'))
qf_save(mask.*single((1-pmap)>0.95).*single(totmap/ntotal<0).*totmap/ntotal*-1,strcat('pNeg_PosiNegFunc_',CRS,'_',lrap1,'_',tp,'.nii.gz'))
