function para= strokeShow_mvgc_3barchart %(maski,lrap,patientiList)
...
patientiList=[11,12,16,17,18,20,21,22,26];
lrap='La'; %La Ra Lp Rp
maski=[8,10,14];
...
for ii=1:length(patientiList)
    iii=iii+1;
        ...
        T1s=load_nii(strcat('tdcs',num2str(ii),'_TP1_zstat1.nii.gz'));
...
% b=T2s.img;
% c=T3s.img;
% al=reshape(a,7221032,1);
% bl=reshape(b,7221032,1);
% ma=mean(al);
% mb=mean(bl);
% sa=std(al);
% sb=std(bl);
...
mask=[];
for ll=1:maSize(2)
    string=maskList_mac{maski(ll)};
    masks=load_nii(string(37:end));
    if isempty(mask)==1
    mask=masks.img;
    else
        mask=mask+masks.img;
