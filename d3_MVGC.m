function [F,pval,sig,morder]=d3_MVGC(X)

%X=cat(2,X(:,[11:18]),X(:,[25:32]),X(:,[39:46]),X(:,[53:60]),X(:,[67:74]));

[~,nobs ,ntrials  ] = size(X);     % number of trials
       % number of observations per trial

regmode   = 'LWR';  %OLS VAR model estimation regression mode ('OLS', 'LWR' or empty for default)
icregmode = 'LWR';  %LWR information criteria regression mode ('OLS', 'LWR' or empty for default)
morder1=1;
